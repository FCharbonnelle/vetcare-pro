...
export default function Dashboard() {
  ...
  const handleAction = React.useCallback((path: string) => {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <Animated.ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.scroll, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
        ...
        <View style={styles.actionsGrid}>
           <QuickAction icon={Zap} label="IA Assist" color="#A855F7" onPress={() => handleAction('/ai-assist')} />
           <QuickAction icon={Activity} label="Santé" color="#10B981" onPress={() => handleAction('/history')} />
           <QuickAction icon={MapPin} label="Trouver" color="#3B82F6" onPress={() => handleAction('/map')} />
        </View>
