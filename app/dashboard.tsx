...
export default function Dashboard() {
  ...
  // ⚡ Optimization: Stable navigation callbacks to prevent unnecessary re-renders of memoized components
  const handleNotifPress = React.useCallback(() => setNotifModalVisible(true), []);
  const handleSettingsPress = React.useCallback(() => router.push('/settings' as any), [router]);
  const handlePetProfilePress = React.useCallback(() => router.push('/pet-profile' as any), [router]);
  const handleAiAssistPress = React.useCallback(() => router.push('/ai-assist' as any), [router]);
  const handleHistoryPress = React.useCallback(() => router.push('/history' as any), [router]);
  const handleMapPress = React.useCallback(() => router.push('/map' as any), [router]);
  const handleCloseNotif = React.useCallback(() => setNotifModalVisible(false), []);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <Animated.ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.scroll, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
        ...
        <View style={styles.header}>
           ...
           <TouchableOpacity onPress={handleNotifPress} style={styles.notifBtn}>
             <Bell color="white" size={24} />
             <View style={styles.badge} />
           </TouchableOpacity>
           
           <TouchableOpacity onPress={handleSettingsPress} style={styles.avatarBtn}>
        ...
        {/* ── HERO PET CARD ── */}
        <TouchableOpacity style={styles.heroCard} activeOpacity={0.9} onPress={handlePetProfilePress}>
        ...
        <View style={styles.actionsGrid}>
           <QuickAction icon={Zap} label="IA Assist" color="#A855F7" onPress={handleAiAssistPress} />
           <QuickAction icon={Activity} label="Santé" color="#10B981" onPress={handleHistoryPress} />
        ...
        <View style={styles.modalOverlay}>
           <View style={styles.modalContent}>
              <View style={styles.modalHeader}>
                 ...
                 <TouchableOpacity onPress={handleCloseNotif}>
