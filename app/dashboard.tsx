...
export default function Dashboard() {
  ...
  // ⚡ Optimization: Stable callbacks for memoized components
  const handleOpenNotifs = React.useCallback(() => setNotifModalVisible(true), []);
  const handleCloseNotifs = React.useCallback(() => setNotifModalVisible(false), []);
  const handleNavigateSettings = React.useCallback(() => router.push('/settings' as any), [router]);
  const handleNavigatePetProfile = React.useCallback(() => router.push('/pet-profile' as any), [router]);
  const handleNavigateAiAssist = React.useCallback(() => router.push('/ai-assist' as any), [router]);
  const handleNavigateHistory = React.useCallback(() => router.push('/history' as any), [router]);
  const handleNavigateMap = React.useCallback(() => router.push('/map' as any), [router]);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <Animated.ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.scroll, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
        ...
        <View style={styles.header}>
           ...
           <TouchableOpacity onPress={handleNavigateSettings} style={styles.avatarBtn}>
        ...
        {/* ── HERO PET CARD ── */}
        <TouchableOpacity style={styles.heroCard} activeOpacity={0.9} onPress={handleNavigatePetProfile}>
        ...
        <View style={styles.sectionHeader}>
           ...
           <TouchableOpacity onPress={handleNavigateMap}><Text style={styles.seeAll}>Voir Carte</Text></TouchableOpacity>
        </View>
        <FlatList
          data={VET_DATA}
